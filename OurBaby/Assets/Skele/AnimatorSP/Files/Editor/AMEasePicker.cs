...
public class AMEasePicker : EditorWindow {
	...
	void OnGUI() {
        this.titleContent = new GUIContent("Ease: " + (oData.time_numbering ? AMTimeline.frameToTime(key.frame, (float)aData.getCurrentTake().frameRate) + " s" : key.frame.ToString()));
    ...
    private List<AnimationCurve> m_curves = new List<AnimationCurve>();  
    void createAllCurves()
    {
        for (int i = 0; i <= 31; ++i)
        {
            AnimationCurve _curve = new AnimationCurve();
            switch ((AMTween.EaseType)i)
            {
